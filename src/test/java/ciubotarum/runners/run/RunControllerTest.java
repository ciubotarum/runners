...
class RunControllerTest {
    ...
    void shouldUpdateRun() throws Exception {
        var run = new Run(null, "test", LocalDateTime.now(), LocalDateTime.now().plus(20, ChronoUnit.MINUTES), 1, Location.INDOOR);
    ...
    public void shouldDeleteRun() throws Exception {
        Run runToDelete = new Run(1, "Monday Morning Run", LocalDateTime.now(),
                LocalDateTime.now().plus(30, ChronoUnit.MINUTES), 3, Location.INDOOR);
        when(repository.findById(1)).thenReturn(Optional.of(runToDelete));
